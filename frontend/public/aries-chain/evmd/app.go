...
type EVMD struct {
	*baseapp.BaseApp
	...
	// pacer flag to bypass empty block state transitions
	isEmptyBlock bool
}
...
func NewExampleApp(
	...
	// enable optimistic execution (disabled to allow synchronous empty-block bypass)
...
func (app *EVMD) BeginBlocker(ctx sdk.Context) (sdk.BeginBlock, error) {
	app.Logger().Info("🔍 [BeginBlocker] execution status", "isEmptyBlock", app.isEmptyBlock, "height", ctx.BlockHeight())
	if app.isEmptyBlock {
...
func (app *EVMD) EndBlocker(ctx sdk.Context) (sdk.EndBlock, error) {
	app.Logger().Info("🔍 [EndBlocker] execution status", "isEmptyBlock", app.isEmptyBlock, "height", ctx.BlockHeight())
	if app.isEmptyBlock {
...
func (app *EVMD) FinalizeBlock(req *abci.RequestFinalizeBlock) (res *abci.ResponseFinalizeBlock, err error) {
	app.isEmptyBlock = len(req.Txs) == 0
	app.Logger().Info("🔍 [FinalizeBlock] execution status", "isEmptyBlock", app.isEmptyBlock, "txCount", len(req.Txs), "height", req.Height)
	res, err = app.BaseApp.FinalizeBlock(req)
	app.isEmptyBlock = false
	return res, err
}
